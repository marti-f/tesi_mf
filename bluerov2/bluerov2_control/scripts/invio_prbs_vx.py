...
class invio_prbs:
  ...
  def write_to_file(self,data):
    data_path = get_pkg_dir('bluerov2_control') + '/config/identificazione.yaml'
  ...
  def read_callback(self,msg):
    self.fx = rnd.choice([-3100, 3100])
    self.input_pub.force.x = self.fx
    self.thruster_pub.publish(self.input_pub)
    output_data = msg.velocity.x
    input_data = self.fx
